...
app.post('/paddle-webhooks', async (req, res) => {
  ...
  switch (paddleEvent.alert_name) {
    ...
    case 'subscription_updated': {
      ...
      const keyrixLicense = await fetch(`https://keyrix-api.focusapps.app/v1/accounts/${KEYRIX_ACCOUNT_ID}/licenses/${key}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${KEYRIX_PRODUCT_TOKEN}`,
          'Accept': 'application/vnd.api+json'
      ...
      const { data: license, errors } = await keyrixLicense.json()
      if (errors) {
      ...
      switch (paddleEvent.status) {
        case 'past_due': {
          ...
          const keyrixLicense = await fetch(`https://keyrix-api.focusapps.app/v1/accounts/${KEYRIX_ACCOUNT_ID}/licenses/${key}/actions/suspend`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${KEYRIX_PRODUCT_TOKEN}`,
              'Accept': 'application/vnd.api+json'
          ...
          const { errors } = await keyrixLicense.json()
          if (errors) {
            res.sendStatus(500)
        ...
        case 'active': {
          ...
          const keyrixLicense = await fetch(`https://keyrix-api.focusapps.app/v1/accounts/${KEYRIX_ACCOUNT_ID}/licenses/${key}/actions/reinstate`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${KEYRIX_PRODUCT_TOKEN}`,
              'Accept': 'application/vnd.api+json'
    ...
    case 'subscription_cancelled': {
      ...
      const keyrixLicense = await fetch(`https://keyrix-api.focusapps.app/v1/accounts/${KEYRIX_ACCOUNT_ID}/licenses/${key}/actions/revoke`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${KEYRIX_PRODUCT_TOKEN}`,
          'Accept': 'application/vnd.api+json'
...
app.post('/keyrix-webhooks', async (req, res) => {
  ...
  switch (keyrixEvent.attributes.event) {
    // 3. Respond to machine creation and deletion events within your Keyrix account. Here,
    ...
    case 'machine.deleted': {
      const { data: keyrixMachine } = JSON.parse(keyrixEvent.attributes.payload)

      // 4. Request the customer's license so that we can look up the correct
      //    Paddle subscription.
      const keyrixLicense = await fetch(`https://keyrix-api.focusapps.app/v1/accounts/${KEYRIX_ACCOUNT_ID}/licenses/${keyrixMachine.relationships.license.data.id}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${KEYRIX_PRODUCT_TOKEN}`,
          'Accept': 'application/vnd.api+json'
      ...
      const { data: license, errors: errs1 } = await keyrixLicense.json()
      if (errs1) {
        res.sendStatus(500)
      ...
      const keyrixMachines = await fetch(`https://keyrix-api.focusapps.app/v1/accounts/${KEYRIX_ACCOUNT_ID}/licenses/${license.id}/machines?page[size]=100&page[number]=1`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${KEYRIX_PRODUCT_TOKEN}`,
          'Accept': 'application/vnd.api+json'
      ...
      const { data: machines, links, errors: errs2 } = await keyrixMachines.json()
      if (errs2) {
        res.sendStatus(500)
